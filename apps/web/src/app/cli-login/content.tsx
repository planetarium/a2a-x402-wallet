...
export function CliLoginContent() {
  ...
  useEffect(() => {
    ...
    getAccessToken().then(async (privyToken) => {
      if (!privyToken) {
        window.location.href = `${callback}?error=no_privy_token`;
        return;
      }
      ...
      if (!res.ok) {
        window.location.href = `${callback}?error=token_exchange_failed`;
        return;
      }
      ...
      <main className="flex min-h-screen flex-col items-center justify-center bg-zinc-50 dark:bg-zinc-950 px-4">
        <div className="w-full max-w-sm">
          ...
          <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 shadow-sm p-6">
            <p className="text-sm text-red-500">Missing callback parameter. Please run <code className="font-mono">a2a-wallet auth login</code> again.</p>
          </div>
        </div>
      </main>
    );
          ...
          <div className="border-t border-zinc-100 dark:border-zinc-800 pt-5">
            ...
            {authenticated && isDelegated && (
              <div className="flex flex-col items-center gap-3 py-2 text-center">
                ...
                <div>
                  <p className="font-medium text-sm">Authorizing CLI...</p>
                  <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">
                    You can close this tab once the CLI confirms login.
                  </p>
                </div>
              </div>
            )}
